...
export default function Sidebar() {
    ...
    const menuItems = [
        { label: "Início", icon: HouseIcon, path: "/" },
    ...
    return (
        ...
        >
            ...
            <div className="flex items-center relative px-4 py-3 border-b border-[#30363d]">
                ...
                <div
                    className={`absolute left-16 transition-all duration-300 ${open ? "opacity-100 visible" : "opacity-0 invisible"
                        }`}
                >
...
function SidebarItem({ icon: Icon, label, open, active, onClick }) {
    return (
        <button
            onClick={onClick}
            ...
            text-sm font-medium tracking-tight transition-all duration-200
                ${active ? "bg-[#CDA327]/10" : "hover:bg-[#161b22]/80"}
            `}
        >
            {/* Barrinha lateral (fora do botão) */}
            {active && (
            ...
            {/* Texto */}
            <span
                className={`absolute left-16 transition-all duration-300 ${open
                    ? `opacity-100 visible ${active ? "text-white" : "text-[#c9d1d9]"
                    }`
                    : "opacity-0 invisible"
                    }`}
            >
